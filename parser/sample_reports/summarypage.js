...
function switchToTranslation(lang){
    ...
    switch(lang) {
        ...
        case 'ko':
            ...
            break;
		case 'es':
            RESOURCES_DICT = LANG_ES_TRANS;
            break;
		case 'it':
            RESOURCES_DICT = LANG_IT_TRANS;
            break;
        default:
...
function initSchedulerGraph(){
	...
	{
		document.getElementsByClassName('scheduler')[0].className += " flex-grow scheduler-active";
		getSeriesData();
		
		var dom = document.getElementById('SchedulerGraph');
...
function initTransactionsTable(){
	if(document.getElementById("TransactionsTable")){
		var transTable = $('#TransactionsTable').DataTable({
			"order": [],
			"lengthMenu": [[-1, 25, 50, 100], [RESOURCES_DICT["ENTRIES_ALL"], "25 "+RESOURCES_DICT["ENTRIES"], "50 "+RESOURCES_DICT["ENTRIES"], "100 "+RESOURCES_DICT["ENTRIES"]]],
			"oLanguage": {
			...
			"dom":
			"<'white-bg'<'d-flex'<'#StatusFilter.item flex-grow'><'#LengthFilter.item flex-grow'l><'item flex-grow'f><'#Legend.item flex-grow d-flex align-items-center justify-content-end'>>>" +
			"<'row'<'col-sm-12'tr>>" +
			"<'row'<'col-sm-12 col-md-5 ps-3 pt-1'i><'col-sm-12 col-md-7'p>>"
			
		});	

		
		$('#StatusFilter').html(
			"<div id='sla-status-filter' class='slaStatus_filter dataTables_length d-flex align-items-center'>"+
			"<label>"+RESOURCES_DICT["SLA_STATUS"]+": </label>"+
